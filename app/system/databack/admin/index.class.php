   ...
   function dotableprearray($tablepre){
	...
	/*上传文件备份*/
	 function dopackupload(){
		...
		if(!file_exists(PATH_WEB.ADMIN_FILE.'/databack/upload'))@mkdir (PATH_WEB.ADMIN_FILE.'/databack/upload', 0777);
		$sqlzip=PATH_WEB.ADMIN_FILE.'/databack/upload/'.$_M[config][met_agents_backup].'_upload_'.date('YmdHis',time()).'.zip';
	...
	public function dogetsql($table='') {
  	...
  	function doallfile(){
	    global $_M;
	    $this->dogetsql();
		if(!file_exists(PATH_WEB.ADMIN_FILE.'/databack/web'))@mkdir (PATH_WEB.ADMIN_FILE.'/databack/web', 0777);

		$sqlzip=PATH_WEB.ADMIN_FILE.'/databack/web/'.$_M[config][met_agents_backup].'_web_'.$con_db_name.'_'.date('YmdHis',time()).'_'.met_rand(6).'.zip';
