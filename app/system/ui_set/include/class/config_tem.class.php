...
class config_tem{
	...
	public function get_config($name){
		...
		if(is_numeric($name)){ 
			...
			$query = "SELECT * FROM {$_M['table']['templates']} WHERE bigclass = (SELECT id FROM {$_M['table']['templates']} WHERE lang='{$this->lang}' AND no = '{$this->no}' AND name='{$name}') ORDER BY no_order";
		}
